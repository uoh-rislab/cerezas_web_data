...
def main(current_base_path_reports, previous_base_path_reports, fic_name_2025, fic_name_2024, dataset_key, 
    current_month_root, current_month_words_root, current_start_date_root, current_end_date_root, current_last_7_days_root,
    previous_start_date_root, previous_end_date_root, previous_last_7_days_root, previous_month_root, previous_month_words_root):
    ...
    df_raw_2025 = pd.read_csv(f"{current_base_path_reports}{fic_name_2025}.csv")

    df_raw_2025["timestamp"] = pd.to_datetime(dict(year=df_raw_2025["Año"], month=df_raw_2025["Mes"], day=df_raw_2025["Dia"], hour=df_raw_2025["Hora"]))

    # === Cargar datos 2024 ===
    df_raw_2024 = pd.read_csv(f"{previous_base_path_reports}{fic_name_2024}.csv")
    df_raw_2024["timestamp"] = pd.to_datetime(dict(year=df_raw_2024["Año"], month=df_raw_2024["Mes"], day=df_raw_2024["Dia"], hour=df_raw_2024["Hora"]))


    # === Filtrar rango equivalente 2024 ===
    start_date_2024 = pd.Timestamp("2024-05-01")
    end_date_2024 = pd.Timestamp(previous_end_date_root)

    df_raw_2024 = df_raw_2024[(df_raw_2024["timestamp"] >= start_date_2024) & (df_raw_2024["timestamp"] <= end_date_2024)]


    with open(f"../locations/sensor_locations_{fic_name_2025}.txt", "r", encoding="utf-8") as f:
    ...
    sensor_cols = [col for col in df_raw_2025.columns if col.startswith("A8") and " " not in col and not any(col.startswith(s) for s in excluded_sensors)]
    hf_cols = [col for col in df_raw_2025.columns if col.endswith("HF") and not any(col.startswith(s) for s in excluded_sensors)]
    pf_cols = [col for col in df_raw_2025.columns if col.endswith("PF") and not any(col.startswith(s) for s in excluded_sensors)]
    ...
    df_may = df_raw_2025[(df_raw_2025["timestamp"] >= start_date) & (df_raw_2025["timestamp"] <= end_date)]
    ...
    daily_pf_table_2024 = create_daily_pf_table(df_raw_2024, 5, previous_year_label)
    daily_pf_table_2025 = create_daily_pf_table(df_raw_2025, 5, current_year_label)

    style_table(daily_pf_table_2024)
    style_table(daily_pf_table_2025)
    ...
    combined_table = Table(
        [[daily_pf_table_2024, '', daily_pf_table_2025]],
    ...
    weekly_table_2024 = create_weekly_table(df_raw_2024, previous_start_date_root, previous_end_date_root, style_centered, previous_year_label)
    weekly_table_2025 = create_weekly_table(df_raw_2025, current_start_date_root, current_end_date_root, style_centered, current_year_label)

    style_table(weekly_table_2024)
    style_table(weekly_table_2025)

    story.append(Paragraph(f"<b>Acumulación semanal de Horas Frío y Porciones de Frío - {previous_year_label}</b>", styles['Heading2']))
    story.append(weekly_table_2024)

    story.append(Paragraph(f"<b>Acumulación semanal de Horas Frío y Porciones de Frío - {current_year_label}</b>", styles['Heading2']))
    story.append(weekly_table_2025)
...
if __name__ == "__main__":
    ...
    # === Fechas equivalentes 2024 ===
    previous_base_path_reports = current_base_path_reports.replace("2025","2024")
    previous_start_date_root  = current_start_date_root.replace("2025","2024")
    previous_end_date_root    = current_end_date_root.replace("2025","2024")
    previous_last_7_days_root = current_last_7_days_root.replace("2025","2024")
    previous_month_root       = current_month_root.replace("2025","2024")
    previous_month_words_root = current_month_words_root.replace("2025","2024")
    ...
    for f in files:
        if f.endswith(".csv") and "fic1" in f:
            ...
            fic_name_2025 = f[start:end]
            dataset_key = f[start+5:end]
            fic_name_2024 = f.replace("2025", "2024")[start:end]

            #print(fic_name_2025)
            #print(fic_name_2024)
            #print(f"{previous_base_path_reports}{fic_name_2024}.csv")

            # Chequea si existe el archivo 2024 antes de agregar
            if os.path.exists(f"{previous_base_path_reports}{fic_name_2024}.csv"):
                #print(fic_name_2025)
                #print(fic_name_2024)
                names.append([fic_name_2025, fic_name_2024, dataset_key])
            else:
                print(f"[INFO] No existe archivo 2024 para {fic_name_2025}, no se generará PDF.")

    print(names)

    # Mostrar resultado
    for fic_name_2025, fic_name_2024, dataset_key in names:
        print(fic_name_2025, fic_name_2024, dataset_key)

        main(current_base_path_reports, previous_base_path_reports, fic_name_2025, fic_name_2024, dataset_key, 
            current_month_root, current_month_words_root, current_start_date_root, current_end_date_root, current_last_7_days_root,
            previous_start_date_root, previous_end_date_root, previous_last_7_days_root, previous_month_root, previous_month_words_root)
