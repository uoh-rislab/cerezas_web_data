...
def add_page_decorations_factory(site_logo_path=None):
    def add_page_decorations(canvas, doc):
        header_logos(canvas, doc, site_logo_path)
        footer(canvas, doc)
...
def create_daily_pf_table(df, last_monday, end_date_root):
    ...
    mask = (df['timestamp'] >= last_monday) & (df['timestamp'] <= end_date_root)
    df_filtered = df.loc[mask]

    # Crear rango de fechas diario
    fechas = pd.date_range(start=last_monday, end=end_date_root, freq='D')
...
def main(base_path_reports, fic_name, dataset_key, month_root, month_words_root, start_date_root, end_date_root, last_monday):
    ...
    filename = site_info[dataset_key]["filename"]
    ...
    with open(f"../locations/daily/{end_date_root[:10]}/sensor_locations_{fic_name}.txt", "r", encoding="utf-8") as f:
        local_vars = {}
    ...
    output_dir = Path(f"../output/pdf_d/{end_date_root[:10]}")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"{month_root}-{end_date_root[8:10]} UOH Cerezas Boletin Diario - {filename}.pdf"

    img_map = output_dir / "map_sensores.jpg"

    img_temp7 = output_dir / "fig_temp_ultimos7.jpg"
    df_last7 = df_may[(df_may["timestamp"] >= last_monday) & (df_may["timestamp"] <= end_date_root)]
    ...
    doc = SimpleDocTemplate(
        str(output_file),
        pagesize=A4,
        title=f"{month_root}-{end_date_root[8:10]} UOH Cerezas Boletin Diario - {site_name}",
    ...
    story.append(Paragraph(f"<b>UOH Cerezas - Boletín Diario Agroclimático</b>", styles['Heading1Center']))
    ...
    story.append(Paragraph(f"<b>Período comprendido: {last_monday[8:10]} al {end_date_root[8:10]} de {month_words_root}</b>", styles['Center']))
    ...
    if (num_sensores > 1):
        ...
        resumen_text = (
            ...
            f"producción de cereza en la Región del Libertador Bernardo O’Higgins, se ha disponibilizado la reportería diaria de "
            ...
            f"La revisión de los datos climáticos de la temporada {month_root[:4]} a la fecha considera la revisión diaria de "
            ...
            f"De esta forma, a continuación se reportan las métricas promedio de los sensores, registradas durante la presente semana ({last_monday[8:10]} al {end_date_root[8:10]} de {month_words_root})"
        )
        ...
        resumen_text = (
            ...
            f"agroclimáticas necesarias para los frutales de hoja caduca como lo son las Horas de Frío y "
            f"Porciones de Frío.<br/><br/>"
            f"De esta forma, a continuación se reportan las métricas del sensor, registradas durante la presente semana ({last_monday[8:10]} al {end_date_root[8:10]} de {month_words_root})"
        )
    ...
    story.append(Paragraph(f"<b>Temperatura (°C) promedio en la presente semana ({last_monday[8:10]} al {end_date_root[8:10]} de {month_words_root})</b>", styles['Heading3']))
    story.append(Image(str(img_temp7), width=480, height=270))

    daily_pf_table = create_daily_pf_table(df_raw, last_monday, end_date_root)  
    ...
    site_logo_path = f"../assets/logos/{dataset_key}.png"
    doc.build(story, onFirstPage=add_page_decorations_factory(site_logo_path),
                     onLaterPages=add_page_decorations_factory(site_logo_path))
...
if __name__ == "__main__":
    ...
    # Ruta a la carpeta
    folder                    = f"../reports/daily/data_{yesterday_str}/"
    current_base_path_reports = f"../reports/daily/data_{yesterday_str}/temp_process_output_"
    ...
    for fic_name, dataset_key in names:
        ...
        main(current_base_path_reports, fic_name, dataset_key, 
            current_month_root, current_month_words_root, current_start_date_root, current_end_date_root, current_last_monday)
        #break
